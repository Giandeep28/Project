...
public class StorageManager {
    ...
    private synchronized void saveToDisk(String path, Booking booking) {
        try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(path, true), java.nio.charset.StandardCharsets.UTF_8))) {
            ...
            String entry = String.format("{\"bookingId\":\"%s\", \"flightId\":\"%s\", \"from\":\"%s\", \"to\":\"%s\", \"contactEmail\":\"%s\", \"passengerName\":\"%s\", \"seatClass\":\"%s\", \"totalFare\":%.2f, \"status\":\"%s\"}\n",
                booking.getBookingId(), booking.getFlightId(), booking.getFrom(), booking.getTo(), booking.getContactEmail(), booking.getPassengerName(), booking.getSeatId(), booking.getTotalFare(), booking.getStatus());
            writer.write(entry);
        } catch (IOException e) {
