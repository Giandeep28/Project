...
export default function Booking({ darkMode }) {
  ...
  const [step, setStep] = useState("PASSENGERS"); // 'PASSENGERS', 'SEATS', 'MEALS', 'EXTRAS', 'PAYMENT'
  ...
  const [stopoverOrder, setStopoverOrder] = useState(null);
  ...
  const handleProceedToSeats = () => {
    if (!contactEmail) {
      setPayError("Contact email is required.");
      return;
    }
    for (let i = 0; i < passengers.length; i++) {
      if (!passengers[i].name || !passengers[i].age || !passengers[i].gender) {
        setPayError(`Please complete all fields for Passenger ${i + 1}.`);
        return;
      }
    }
    setPayError(null);
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      // Handle Stopover Order if selected in Extras
      let finalStopoverOrder = null;
      if (extras.includes("dining") && stopoverOrder) {
        try {
          const stopoverRes = await fetch(
            "http://localhost:8080/api/stopover/order",
            {
              method: "POST",
          ...
          if (stopoverRes.ok) {
            finalStopoverOrder = await stopoverRes.json();
          }
        } catch (e) {
          console.error("Stopover order failed (silent):", e);
        }
      }
      ...
      const enrichedBooking = {
        ...result,
        passengers: augmentedPassengers,
        stopoverOrder: finalStopoverOrder,
        legs: flight.legs || [
          {
            from: flight.origin,
            to: flight.destination,
  ...
  if (step === "EXTRAS") {
    return (
      <Extras
        ...
        onConfirm={(selectedExtras, cost, orderData) => {
          setExtras(selectedExtras);
          setExtrasPrice(cost);
          setStopoverOrder(orderData);
          setStep("PAYMENT");
        }}
        darkMode={darkMode}
      />
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px" }}>
        ...
        >
          <span style={{ color: step === "PASSENGERS" ? gold : muted }}>
            1. Passengers
          </span>
          <span style={{ color: muted }}>{">"}</span>
          <span style={{ color: step === "SEATS" ? gold : muted }}>
            2. Seats
          </span>
          <span style={{ color: muted }}>{">"}</span>
          <span style={{ color: step === "MEALS" ? gold : muted }}>
            3. Meals
          </span>
          <span style={{ color: muted }}>{">"}</span>
          <span style={{ color: step === "EXTRAS" ? gold : muted }}>
            4. Extras
          </span>
          <span style={{ color: muted }}>{">"}</span>
          <span style={{ color: step === "PAYMENT" ? gold : muted }}>
        ...
        {step === "PASSENGERS" && (
          ...
          >
            ...
            {passengers.map((p, i) => (
              ...
              >
                <input
                  ...
                  value={p.name}
                  onChange={(e) => {
                    const neo = [...passengers];
                    neo[i].name = e.target.value;
                    setPassengers(neo);
                  }}
                  style={{
                    background: inputBg,
            ...
            >
              ...
              <input
                type="email"
                placeholder="Email Address for Tickets"
                value={contactEmail}
          ...
          >
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span>
                  Base Flight Fare ({passengers.length} Passenger
            ...
            <div style={{ display: "flex", gap: 16 }}>
              ...
              <button
                ...
                style={{
                  flex: 2,
                  padding: "18px 0",
                  background: payLoading ? "#9ca3af" : gold,
                  color: payLoading ? "#fff" : "#000",
                  border: "none",
                  borderRadius: 10,
                  fontSize: 14,
                  fontWeight: 800,
